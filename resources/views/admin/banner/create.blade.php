<x-app-layout>
    ...
    <div
        ...
        <form class="bg-white w-full space-y-4" method="post" action="{{ route('save.banner') }}"
            ...
            <!-- Type -->
            <div>
                <label class="block">Banner Type<span class="text-red-700">*</span>
                    <select name="type" class="w-full mt-1 p-2 border rounded">
                        <option value="image" {{ old('type') == 'image' ? 'selected' : '' }}>Image</option>
                        <option value="video" {{ old('type') == 'video' ? 'selected' : '' }}>Video</option>
                    </select>
                    @if ($errors->has('type'))
                        <span class="mt-1 text-sm text-red-500">{{ $errors->first('type') }}</span>
                    @endif
                </label>
            </div>
            ...
            <div>
                <label class="block">
                    ...
                    <input id="imageInput" name="image" type="file"
                        class="w-full mt-1 p-2 border rounded" onchange="previewImage(event)">
                </label>
            ...
            <div>
                <label class="block">
                    ...
                    <input type="url" name="video_url" value="{{ old('video_url') }}"
                        class="w-full mt-1 p-2 border rounded" placeholder="https://www.youtube.com/embed/...">
                </label>
            </div>
