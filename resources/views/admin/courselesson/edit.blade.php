<x-app-layout>
    ...
    <div
        ...
        <form class="bg-white w-full space-y-4" method="post"
            action="{{ route('update.courselessons', $courselesson->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div>
                <label class="block">
                    <span class="text-gray-700">Course Module ID</span></label>
                <select name="course_module_id" class="w-full mt-1 p-2 border rounded">
                    ...
                    @foreach ($coursemod as $row)
                        <option value="{{ $row->id }}"
                            {{ $courselesson->course_module_id == $row->id ? 'selected' : '' }}>
                            {{ $row->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input name="title" type="text" value="{{ $courselesson->title }}"
            ...
            <div>
                ...
                <select name="type" id="lessonType" class="w-full mt-1 p-2 border rounded">
                    ...
                    <option value="video" {{ old('type', $courselesson->type) == 'video' ? 'selected' : '' }}>Video
                    </option>
                    <option value="text" {{ old('type', $courselesson->type) == 'text' ? 'selected' : '' }}>Text
                    </option>
                    <option value="quiz" {{ old('type', $courselesson->type) == 'quiz' ? 'selected' : '' }}>Quiz
                    </option>
                    <option value="downloadable"
                        {{ old('type', $courselesson->type) == 'downloadable' ? 'selected' : '' }}>Downloadable
                    </option>
                </select>
            </div>
            ...
            <div>
                <label class="block">
                    ...
                    <textarea name="content" id="" rows="5" class="w-full mt-1 p-2 border rounded summernote"
                        placeholder="Enter Content">{{ $courselesson->content }}</textarea>
                </label>
            </div>
            ...
            <div>
                <label class="block">
                    ...
                    <input type="text" name="video_url" value="{{ old('video_url', $courselesson->video_url) }}"
            ...
            <div>
                <label class="block">
                    ...
                    @if ($courselesson->downloadable_file)
                        <p class="mt-2 text-sm text-gray-600">
                            Current: <a href="{{ asset('uploads/' . $courselesson->downloadable_file) }}"
                                target="_blank" class="text-blue-600 underline">Download File</a>
                        </p>
                    @endif
                </label>
            </div>
            ...
            <div>
                ...
                <input name="order_no" type="number" value="{{ $courselesson->order_no }}"
