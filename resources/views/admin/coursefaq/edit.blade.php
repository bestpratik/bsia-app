<x-app-layout>
    ...
    <div
        ...
        <form class="bg-white w-full space-y-4" method="post" action="{{ route('update.coursefaq', $coursefaq->id) }}"
            ...
            <div>
                ...
                <select name="course_id" class="w-full mt-1 p-2 border rounded">
                    ...
                    @foreach ($course as $row)
                        <option value="{{ $row->id }}" {{ $coursefaq->course_id == $row->id ? 'selected' : '' }}>
                            {{ $row->title }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input name="title" type="text" value="{{ $coursefaq->title }}"
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full mt-1 p-2 border rounded summernote">{{ $coursefaq->description }}</textarea>
            </div>

            <!-- Order No -->
            <div>
                <label class="block">Order No</label>
                <input name="order_no" type="number" value="{{ $coursefaq->order_no }}"
