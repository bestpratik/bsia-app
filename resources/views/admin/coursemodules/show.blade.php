<x-app-layout>
    <div class="max-w-4xl mx-auto p-8 bg-white rounded-2xl shadow-lg border border-gray-200">

        <!-- Header -->
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 border-b pb-4">
            <h2 class="text-3xl font-bold text-gray-800 flex items-center gap-2">
                <x-heroicon-o-book-open class="w-8 h-8 text-blue-600" />
                {{ $coursemod->name }}
            </h2>
            <a href="{{ route('coursemodules') }}"
                class="mt-4 sm:mt-0 px-5 py-2.5 text-sm font-medium text-blue-700 hover:text-white border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg transition duration-200">
                <x-heroicon-o-arrow-left class="w-4 h-4 inline-block" /> Back to List
            </a>
        </div>

        <!-- Details Card -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">

            <!-- Name -->
            <div class="p-5 rounded-xl bg-gray-50 border">
                <p class="mb-2 text-sm text-gray-500">Module Name</p>
                <p class="text-lg font-semibold text-gray-800">{{ $coursemod->name ?? 'N/A' }}</p>
            </div>

            <!-- Order No -->
            <div class="p-5 rounded-xl bg-gray-50 border">
                <p class="mb-2 text-sm text-gray-500">Order No</p>
                <p class="text-gray-800">{{ $coursemod->order_no ?? 'N/A' }}</p>
            </div>

            <!-- Description -->
            <div class="md:col-span-2 p-5 rounded-xl bg-gray-50 border">
                <p class="mb-2 text-sm text-gray-500">Description</p>
                <p class="text-gray-700 leading-relaxed">
                    {{ $coursemod->description ?? 'N/A' }}
                </p>
            </div>

            <!-- Status -->
            <div class="p-5 rounded-xl bg-gray-50 border">
                <p class="mb-2 text-sm text-gray-500">Status</p>
                <span
                    class="px-3 py-1 text-sm font-medium rounded-full 
                    {{ $coursemod->status ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                    {{ $coursemod->status ? 'Active' : 'Inactive' }}
                </span>
            </div>

        </div>

        <!-- Footer Buttons -->
        <div class="flex gap-3 mt-10">
            <a href="{{ route('edit.coursemodules', $coursemod->id) }}"
                class="px-6 py-2.5 bg-blue-600 text-white rounded-lg shadow hover:bg-blue-700 transition">
                <x-heroicon-o-pencil-square class="w-5 h-5 inline-block mr-1" />
                Edit Module
            </a>
            <form action="{{ route('delete.coursemodules', $coursemod->id) }}" method="POST"
                onsubmit="return confirm('Delete this course module?')">
                @csrf
                @method('DELETE')
                <button type="submit"
                    class="px-6 py-2.5 bg-red-600 text-white rounded-lg shadow hover:bg-red-700 transition">
                    <x-heroicon-o-trash class="w-5 h-5 inline-block mr-1" />
                    Delete
                </button>
            </form>
        </div>

    </div>
</x-app-layout>
